...
// TODO make this support RB classic and open rally
var Io = Class({

  importGPX: function(gpx, reverse){
    try {
    ...
    this.importGPXTracks($.makeArray(this.gpx.find( "trkpt" )), reverse);
  ...
  exportGPX: function(){
    ...
    for(var i=0;i<points.length;i++){
      if(points[i].instruction){
        ...
        var waypoint = "<wpt lat='" + points[i].getPosition().lat() + "' lon='" + points[i].getPosition().lng() + "'><name>" + name + "</name><desc>" + desc + "</desc>" + this.buildOpenRallyExtensionsString(wptCount,points[i].instruction) + "</wpt>";
        waypoints += waypoint;
        wptCount++;
      }
  ...
  */
  buildDescString: function(count,waypoint) {
    ...
    if(waypoint.notification){
      var type = waypoint.notification.type
      // TODO Speed Zone
      string = (type == "wpm" ? waypoint.notification.bubble : (type == "wps" ? "!!!" : (type == "dsz" ? "SZ"+waypoint.notification.modifier : "")));
    }
    return string;
  },

  importGPXTracks: function(tracks, reverse){
    if(tracks.length > 0){
      var tracks = this.processGpxTracksForImport(tracks, reverse);
  ...
  parseGpxTracksToArray: function(gpxTracks, reverse){
    var tracks = []
    for(var i=0;i<gpxTracks.length;i++){
      let point = {lat: parseFloat($(gpxTracks[i]).attr('lat')), lng: parseFloat($(gpxTracks[i]).attr('lon'))}
      // tracks.push(point);
      tracks = this.appendOrPushTrack(point, tracks, reverse);
    }
    return tracks;
  },

  appendOrPushTrack: function(point, tracks, reverse=false){
    var tracks = Array.from(tracks);
    if(reverse){
      tracks.unshift(point);
    }else {
      tracks.push(point);
    }
    return tracks;
  ...
  processGpxTracksForImport: function(tracks, reverse){
    this.tracks = this.parseGpxTracksToArray(tracks, reverse);
