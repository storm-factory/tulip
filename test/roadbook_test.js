...
QUnit.test("Roadbook Create Waypoints from JSON", function( assert ) {
  var roadbook = roadbookFactory();

  assert.equal(roadbook.waypoints().length, 2, "It can add waypoints from JSON")
});

QUnit.test("Roadbook changeEntryTrackType", function( assert ) {
  var roadbook = roadbookFactory();

  var waypointOneInitialTrack = roadbook.waypoints()[0].exitTrackType;
  var waypointTwoInitialTrack = roadbook.waypoints()[1].entryTrackType;
  roadbook.currentlyEditingWaypoint  = roadbook.waypoints()[1];

  roadbook.changeEditingWaypointEntry('offPiste');
  assert.ok(tracksAlteredCorrectly(roadbook.waypoints()[0], waypointOneInitialTrack, roadbook.waypoints()[1], waypointTwoInitialTrack), "It changes exit of previous waypoint")

  roadbook.currentlyEditingWaypoint  = roadbook.waypoints()[0];

  roadbook.changeEditingWaypointEntry('road');
  assert.equal(roadbook.waypoints()[0].entryTrackType, 'road', "It doesn't error for first waypoint")
});

QUnit.test("Roadbook changeExitTrackType", function( assert ) {
  var roadbook = roadbookFactory();
  ...
  roadbook.changeEditingWaypointExit('road');
  assert.ok(tracksAlteredCorrectly(roadbook.waypoints()[0], waypointOneInitialTrack, roadbook.waypoints()[1], waypointTwoInitialTrack), "It changes entry of the next waypoint")

  roadbook.currentlyEditingWaypoint  = roadbook.waypoints()[1];

  roadbook.changeEditingWaypointExit('road');
  assert.equal(roadbook.waypoints()[0].exitTrackType, 'road', "It doesn't error for last waypoint")
});

// TODO write these tests then features
QUnit.test("Roadbook changeEntryTrackType changes all track types until it reaches a change", function( assert ) {assert.expect(0)})
// TODO write these tests then features
QUnit.test("Roadbook changeExitTrackType changes all track types until it reaches a change", function( assert ) {assert.expect(0)})
